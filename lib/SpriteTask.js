...
SpriteTask.prototype.init = function() {
    ...
    var result = filterCSSRules(this); //[cssResult, unspriteImages]
    
    // 没有被合并的图片会被拷贝到目标目录
    if(this.env.unspriteImages){
        this.env.unspriteImages = [];
    }
    this.env.unspriteImages.concat(result[1]);

    // 4. 并读取图片 info, 同时 cache 住
    readImagesInfo(this, result[0], function(result){
        context.emit('inited');
    });
};
...
function filterCSSRules(task, callback) {
    ...
    return [cssResult, unspriteImages];
}
 ...
 */
function readImagesInfo(task, cssRules){
    cssRules.forEach(function(cssRule){
