...
SpriteTask.prototype.init = function() {
    ...
    this.cssSheet.cssRules = result[0];  //更新cssRules为只需要合并的cssRules
    ...
    this.imageInfos = collectImageRules(result[0]); 
...
SpriteTask.prototype.exec = function() {
    ...
    //     console.log(this.spriteArray[i])
    // }
...
function filtercssRules(task, callback) {
    ...
    return [toSpriteRules, unspriteImages];
}
...
function readImagesInfo(task, callback){
    var cssRules = task.cssSheet.cssRules,
        config = task.env.config,
        cache = task.env.cache;

    var imageInfos = task.imageInfos;
