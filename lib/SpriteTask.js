...
SpriteTask.prototype.init = function() {
    ...
    this.cssSheet = new CSSSheet(config, this.cssUri);
    ...
    if(unspriteImages.length > 0){
        this.env.unspriteImages = us.union(this.env.unspriteImages, unspriteImages);
    }
...
function filtercssRules(task, callback) {
    ...
    cssRules.forEach(function(cssRule) {
        ...
        if (match.length === 2) {
            ...
            var cssDirname = path.dirname(task.cssSheet.filepath);
...
function readImagesInfo(task, callback){
    ...
    zTool.forEach(imageInfos, function(imageInfo, imagePath, next) {
        ...
        var styleSheetDir = path.dirname(task.cssSheet.uri),
        ...
        if(cache[filepath]){
            //曾经处理过cache住的就直接赋值,注意要更换这次的cssRules
            //task.imageInfos[imagePath] = zTool.extend(imageInfo, cache[filepath], false)

            var cacheCssRules = cache[filepath].cssRules;
            cache[filepath].cssRules = cacheCssRules.concat(imageInfo.cssRules);
            task.imageInfos[imagePath] = cache[filepath];
...
function exportCssFile(spriteTask){
    ...
    if(spriteConfig.output.combineCSSRule){
        ...
        for(var imageName in combinedCssRules){ 
            var uniqCssRules = us.uniq( combinedCssRules[imageName] ); //去重        
