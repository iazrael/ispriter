...
SpriteTask.prototype.init = function() {
    ...
    var result = filtercssRules(this); //[cssRules, unspriteImages]
    var cssRules = result[0],
        unspriteImages = result[1];

    // 没有被合并的图片会被拷贝到目标目录
    if(!this.env.unspriteImages){
        this.env.unspriteImages = unspriteImages;
    }

    // 4. 并读取图片 info, 同时 cache 住
    readImagesInfo(this, cssRules, function(){
        context.emit('inited');
    });
};
...
SpriteTask.prototype.exec = function() {
    ...
    // 3. 输出合并的后的大图
    // 4. 输出修改后的样式表
    var spriteImage = new SpriteImage(this.env.config, this.cssSheet);
    this.spriteArray = spriteImage.positionImages();

    info(this.spriteArray)

    // // spriteArray 的每个元素就是每张精灵图
    // var spriteArray = positionImages(this.styleObjList);
    //     this.spriteArray = spriteArray;

    // // 输出合并的图片 并修改样式表里面的background
    // drawImageAndPositionBackground(this);
...
function filtercssRules(task, callback) {
    ...
    return [newCssRules, unspriteImages];
}

/**
 * 读取所有 cssRules 的图片信息 包括图片的长宽大小
 * @param  {[SpriteTask]} task     [description]
 * @param  {[Array]} cssRules [description]
 * @param  {[Function]} callback [description]
 * @return {[type]}          [description]
 */
function readImagesInfo(task, cssRules, callback){

    cssRules.forEach(function(cssRule,index){
        var spriteImage = cssRule.__spriteImage,
            img = new iImage();  //注意这不是个图片的Image对象

        //info('[cssRule]',cssRule)
        var config = task.env.config,
            cache = task.env.cache;
        var styleSheetDir = path.dirname(config.input.cssSource),
            imageAbsUrl = path.join(styleSheetDir,spriteImage.uri),
            fileName = path.join(config.workspace, imageAbsUrl);

        var _checkThisFuncOver = function(){
            if(index+1 == cssRules.length){
                info('--- All ImageInfos were geted! ---');
                callback();
            }
        }
        ...
        //曾经处理过cache住的就直接continue,在forEach里面是return
        if(cache[fileName]){
            _checkThisFuncOver();
            return;
        }
        ...
        img.src = fileName;
        img.on('load',function(imageInfo){
            //方便查看信息的正确与否
            var loggerInfo = {
            ...
            debugInfo('[imageInfo]',loggerInfo);

            //(很有必要，里面还设置了必要参数w和h)从所有style里面，选取图片宽高最大的作为图片宽高
            setImageWidthHeight(cssRules, imageInfo);

            cache[fileName] = imageInfo;
            cssRule.imageInfo = imageInfo;  //TODO 看放到cssRule好用，还是全局cache好用
 ...
 */
function setImageWidthHeight(cssRules, imageInfo){
    var w = 0, 
        h = 0, 
        mw = imageInfo.width, 
        mh = imageInfo.height
    ...
    // 遍历所有规则, 取最大值
    cssRules.forEach(function(cssRule){
        w = cssRule.__narmalizer.width,
        h = cssRule.__narmalizer.height;
     ...
     */
    cssRule.w = mw + spriteConfig.output.margin;
    cssRule.h = mh + spriteConfig.output.margin;
