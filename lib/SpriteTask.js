...
function normalizeCSSRule(config, cssRule) {
    ...
    if (value = cssRule['width']) {
        normalizer['width'] = parseFloat(value);
    }
    if (value = cssRule['height']) {
        normalizer['height'] = parseFloat(value);
    }
    if (value = cssRule['background-repeat']) {
        normalizer['background-repeat'] = value;
    }
    if (value = cssRule['background-image']) {
        normalizer['background-image'] = value;
    }
    ...
    if (value = cssRule['background']) {
        ...
        var bg = BgInterpreter.analyse(value);
        if (bg.length !== 1) {

            // FIXME 暂时跳过一个 background 里面简写了多个背景的情况
            // 这种情况一般也比较少见, 如果遇到了, 就取第一个有 background-image 的

            for (var i = 0, item; i < bg.length; i++) {
                item = bg[i];
        ...
        normalizer = us.extend({}, normalizer, bg[0]);
    }
    return normalizer;
...
function filterCSSRules(task, callback) {
    ...
    cssRules.forEach(function(cssRule) {
        ...
        var match = resolveBackgroundImage(config, cssRule['background-image']);
