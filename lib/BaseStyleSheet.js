 ...
 * }
 */
function collectStyleRules(styleSheet, result, styleSheetUri){
// TODO 下一步, 从这里开始
    if(!result){
    ...
    var styleSheetDir = path.dirname(styleSheetUri);
    ...
    styleSheet.cssRules.forEach(function(rule){
        ...
        if(rule.href && rule.styleSheet){
            ...
            var cssUri = path.join(styleSheetDir, fileName);
            var styleSheet = readStyleSheet(cssUri);
            Logger.debug('read import style: ' + cssUri + ' , has styleSheet == : ' + !!styleSheet);
         ...
         * 给 style 对象扩展基本的方法
         */
        var style = BaseCSSStyleDeclaration.inherits(rule.style);
        ...
        if(constant.REGEXP_IGNORE_POSSITION.test(style['background-position-x']) || 
            constant.REGEXP_IGNORE_POSSITION.test(style['background-position-y'])){
        ...
        if(constant.REGEXP_IGNORE_REPEAT.test(style['background-repeat']) || 
            constant.REGEXP_IGNORE_REPEAT.test(style['background-repeat-x']) || 
            constant.REGEXP_IGNORE_REPEAT.test(style['background-repeat-y'])){
