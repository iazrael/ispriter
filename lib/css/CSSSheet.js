...
function pickupCSSRules(cssText){
    ...
    var cssRules = [];
    var index, start, end, classEnd, cssStr, cssName;
    ...
    while(pos++ < length){
        ...
        // 把已经处理过的文本切出来
        tmpText = cssText.substring(pos); // .name { color: #fff; background: xxx }

        // 把 background 找出来
        index = tmpText.indexOf('background'); // .name { color: #fff; []background: xxx }
        if(index === -1){
            break;
        }
        ...
        // 向前 / 向后找到样式闭合点和样式名字
        // 向前找 {
        tmpText2 = tmpText.substring(0, index); // .name { color: #fff; 
        start = tmpText2.lastIndexOf('{'); // .name []{ color: #fff; 
        cssStr = tmpText2.substring(start); // { color: #fff;  

        // 继续向前找样式名字
        tmpText2 = tmpText2.substring(0, start); // .name 

        // 找到前一个样式的结束符, 中间的就是名字
        start = tmpText2.lastIndexOf('}');

         // +1 是为了跳过 } 
        cssName = tmpText2.substring(start + 1); // .name 

        // 向后找 }
        tmpText2 = tmpText.substring(index); // background: xxx }
        end = tmpText2.indexOf('}'); // background: xxx []}
        classEnd = index + end;

        // +1 是为了包括 }
        cssStr += tmpText2.substring(0, end + 1); // background: xxx }

        // pos 是指在完整 cssText 中的位置
        cssRules.push(new CSSRule(cssName, cssStr, pos + start + 1, classEnd + 1 ));
